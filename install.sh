...
cp "$SCRIPT_DIR/bin/claude-hud" "$BIN_DIR/"
cp "$SCRIPT_DIR/bin/hud-status" "$BIN_DIR/"
chmod +x "$BIN_DIR/claude-hud"
chmod +x "$BIN_DIR/hud-status"
ln -sf "$BIN_DIR/claude-hud" "$BIN_DIR/hud"
echo -e "  ${GREEN}✓${NC} CLI tools installed (hud, claude-hud, hud-status)"
